...
+ [调整] 画布和视频工作台的视频任务轮询超时统一延长至 1 小时，网络、服务端失败和手动停止仍会立即结束。
+ [修复] 视频自动引用角色、道具和场景卡时按完整卡名消歧，避免名称前缀相同的场景被同时引用。
+ [调整] 视频提示词中的卡片引用改为实际参考图片标签，并在标签后说明对应角色、道具或场景名称。
+ [优化] 角色、道具和场景卡使用明确的卡头拖动区，支持多选后一起移动。
+ [新增] 视频生成前会提示已引用但缺少图片的角色、场景和道具卡，并支持定位补充或无视继续生成。
+ [调整] 视频提示词引用角色、道具和场景卡时，会在开头自动补充对应图片的身份或环境介绍。
+ [调整] 画布左侧资产页按角色卡、场景卡、道具卡和素材四类展示内容。
+ [调整] 新建视频节点沿用上一次视频设置的模型、画幅、时长、清晰度、音频和水印参数。
+ [优化] 角色、道具和场景卡增加顶部拖动区，名称输入框仅在悬浮时显示。
+ [调整] 角色、道具和场景卡单击可添加主图片素材，按住并拖动可移动单张或多选卡片。
+ [调整] 多选等距排列支持角色、道具和场景卡与图片、视频混合排布。
+ [修复] 角色、道具和场景卡可从非编辑区域正常拖动。
+ [新增] 画布新增角色、道具和场景卡，可将卡片素材自动引用并连线到提及其名称的视频节点。
+ [优化] 视频节点的参考图片缩略图支持一键以 `@` 形式加入提示词。
+ [新增] 画布支持将多选图片和视频以 16 像素间距横向上对齐、纵向右对齐或三列宫格排布。
+ [调整] 画布空白处改为左键框选，需按住空格和左键才可平移画布。

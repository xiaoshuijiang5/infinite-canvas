...
import { BookOpen, Check, ChevronRight, Download, Drama, Eye, FileText, Image as ImageIcon, ListChecks, MapPinned, Music2, Package, Plus, Search, Settings2, Square, Trash2, Type, Video } from "lucide-react";
...
const NODE_TYPE_ICON: Record<string, typeof Square> = {
    ...
    [CanvasNodeType.CharacterCard]: Drama,
    [CanvasNodeType.PropCard]: Package,
    [CanvasNodeType.SceneCard]: MapPinned,
};
...
const NODE_FILTER_VALUES = ["all", CanvasNodeType.Image, CanvasNodeType.Video, CanvasNodeType.Text, CanvasNodeType.Audio, CanvasNodeType.Config, CanvasNodeType.Group, CanvasNodeType.CharacterCard, CanvasNodeType.PropCard, CanvasNodeType.SceneCard];

function nodePreviewText(node: CanvasNodeData) {
    if ([CanvasNodeType.CharacterCard, CanvasNodeType.PropCard, CanvasNodeType.SceneCard].includes(node.type as CanvasNodeType)) return node.metadata?.cardName || getNodeDefinition(node.type)?.title || node.type;
                                ...
                                <div key={node.id} className={cn("group relative flex items-center rounded-lg transition", depth && "ml-5", active ? "" : "hover:bg-black/5 dark:hover:bg-white/5")} style={active ? { background: theme.toolbar.activeBg } : undefined}>
                                    ...
                                    <button type="button" onClick={() => (selectMode ? toggleChecked(node.id) : onFocusNode(node.id))} className={cn("flex min-w-0 flex-1 items-center gap-3 py-2 pr-2 text-left", node.type === CanvasNodeType.Group && hasChildren ? "pl-0" : "pl-2")} title={selectMode ? undefined : t("canvas.sidePanel.focusNode")}>
                                        ...
                                        <span className="grid size-10 shrink-0 place-items-center overflow-hidden rounded-md">
                                            {isImage ? <img src={previewImage} alt={node.title} className="size-full object-cover" /> : <Icon className="size-5 opacity-60" />}
                                        </span>
